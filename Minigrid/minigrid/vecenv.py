...
class MiniGridVecEnv(VecEnv):
    ...
    def __init__(self, env_fns: list[Callable[[], gym.Env]]):
        ...
        self.buf_obs = OrderedDict([(k, np.zeros((self.num_envs, *tuple(shapes[k])), dtype=dtypes[k])) for k in self.keys[:-1]] + [(k, np.zeros((self.num_envs))) for k in self.keys[-1:]])
